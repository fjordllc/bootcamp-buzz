...
さらに、X（旧Twitter）で検索しながらバズを登録していくときは、**すでに登録済みのポストに自動で「✓ 登録済み」の目印**が付くので、二重チェックを防げます（[6. X（旧Twitter）で登録済みのポストに印を付ける](#6-x旧twitterで登録済みのポストに印を付ける)）。
...
5. [使い方](#5-使い方)
6. [X（旧Twitter）で登録済みのポストに印を付ける](#6-x旧twitterで登録済みのポストに印を付ける)
...
## 6. X（旧Twitter）で登録済みのポストに印を付ける

X で「フィヨルドブートキャンプ」「FBC」「FjordBootCamp」などを検索してバズを登録していくとき、**同じポストを二重にチェックしなくて済むように**、すでに登録済みのポストには自動で目印が付きます。

### どう見えるか

- X（`x.com` / `twitter.com`）のページで、**すでに登録済みのポストには緑色の「✓ 登録済み」バッジ**が表示されます。
- ポストをいちいち開かなくても、検索結果やタイムラインを眺めるだけで「これはもう登録した」とひと目でわかります。
...
- 目印が付くのは **X のポストそのものの URL** を登録している場合です。ポスト内のリンク先（外部のブログ記事など）の URL を登録している場合は対象外です。
